...
class Layout extends React.Component {
  ...
  newEdge(e) {
    if (e.which === 13) {
      ...
      if (edgeVals.length >= 3) {
        ...
        this.setState({ error: 'Aresta inválida!' })
      }
    }
  }
  ...
  render() {
    ...
    const edgesMapped = edges.map(edge => (
      <li className="edge_list" key={edge.edge_id}>
        ( {edge.u} , {edge.v} )
      </li>
    ))
    const dfs_output = dfs.map(v => (
      <li className="vertex" key={v}>
        {v}
      </li>
    ))
    const DFS = dfs.map(v => (
      <li className="dfs_list" key={v}>
        {v}
      </li>
    ))
    return (
      <div className="mainDiv">
        <table className="tab">
          <tbody>
            ...
            <tr>
              <td>
                <input className="edge" onKeyPress={this.newEdge.bind(this)} />
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <input
          className="startV"
          onChange={this.sendVertex.bind(this)}
          placeholder="Nó inicial"
        />
        <button className="dfs" onClick={this.dfsStart.bind(this)}>
          Iniciar
        </button>
        <hr className="line" />
        <h1>DFS Output</h1>
        ...
        <br />
        <ul id="nodes" className="list_dfs">
          {dfs_output}
        </ul>
      </div>
    )
